...
class QACER(BaseACERAgent):
    ...
    def _learn_from_experience_batch(self, obs, obs_next, actions, old_policies,
        ...
        expected_value_next = tf.reduce_mean(values_next, axis=1)

        u = tf.expand_dims(rewards, axis=1) + self._gamma * values_next - values
        ...
        if self._kappa > 0.:
            q_loss = tf.abs(tau - tf.cast(tf.less(u, 0), tf.float32)) * tf.where(tf.abs(u) < self._kappa, u, self._kappa * tf.sign(u))
        else:
            q_loss = tf.abs(tau - tf.cast(tf.less(u, 0), tf.float32))
        q_loss = tf.squeeze(u * q_loss)

        # flat tensors
        d1_coeffs = gamma_coeffs * (rewards + self._gamma * expected_value_next - expected_value) \
            * truncated_densities.flat_values
        # ragged
        ...
        # flat tensors
        d2_coeffs = tf.expand_dims(gamma_coeffs, axis=1) * q_loss \
            * tf.expand_dims(truncated_densities.flat_values, axis=1)
        # ragged
    ...
    def _backward_pass(self, observations: tf.Tensor, actions: tf.Tensor, d1: tf.Tensor, d2: tf.Tensor):
        ...
        with tf.GradientTape() as tape:
            loss = self._critic.loss(observations, d2)
